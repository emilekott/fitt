...
<form action="<?php echo hikashop_completeLink('order'); ?>" method="post" name="adminForm">
	...
	<table class="hikashop_orders adminlist" cellpadding="1">
		...
		<tbody>
			<?php
				$orderUrl = hikashop_completeLink('order');
					...
					<td class="hikashop_order_status_value">
						...
						<?php if(!empty($row->show_payment_button) && bccomp($row->order_full_price,0,5)>0){ ?>
							<form action="<?php echo $orderUrl; ?>" method="post" name="adminForm_<?php echo $row->order_id; ?>">
								...
								$url = hikashop_completeLink('order&task=pay&order_id='.$row->order_id);
						...
						if( isset($row->show_cancel_button) && $row->show_cancel_button ) {?>
							<form action="<?php echo hikashop_completeLink('order'); ?>" method="post" name="adminForm_<?php echo $row->order_id; ?>_cancel">
								<?php
								$text = JText::_('CANCEL_ORDER');
								echo $this->cart->displayButton($text,'cancel_order',$this->params,hikashop_completeLink('order&task=cancel_order&email=1&order_id='.$row->order_id),'document.adminForm_'.$row->order_id.'_cancel.submit();return false;','class="hikashop_order_cancel_button"'); ?>
								<input type="hidden" name="Itemid" value="<?php echo $Itemid; ?>"/>
								<input type="hidden" name="option" value="<?php echo HIKASHOP_COMPONENT; ?>" />
