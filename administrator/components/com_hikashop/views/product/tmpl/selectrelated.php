...
<fieldset>
	<div class="toolbar" id="toolbar" style="float: right;">
		<button type="button" onclick="if(document.adminForm.boxchecked.value==0){alert('<?php echo JText::_( 'PLEASE_SELECT_SOMETHING',true ); ?>');}else{submitbutton('addrelated');}"><img src="<?php echo HIKASHOP_IMAGES; ?>add.png"/><?php echo JText::_('OK'); ?></button>
	</div>
</fieldset>
	...
	<tr>
		<?php if($this->config->get('category_explorer')){?>
			<td style="vertical-align:top;border:1px solid #CCC;background-color: #F3F3F3" width="200px">
				<?php echo hikashop_setExplorer('product&task=selectrelated&select_type='.$this->type,$this->pageInfo->filter->filter_id,true,'product'); ?>
			</td>
		<?php } ?>
		<td style="vertical-align:top;">
			<form action="index.php?option=<?php echo HIKASHOP_COMPONENT ?>&amp;ctrl=product" method="post" name="adminForm">
				<table>
					<tr>
						<td width="100%">
							<a href="<?php echo hikashop_completeLink('product&task=selectrelated&filter_id=0&select_type='.$this->type,true); ?>"><?php echo JText::_( 'ROOT' ); ?>/</a>
							...
							<tr class="<?php echo "row$k"; ?>">
								...
								<td align="center">
									<?php if(in_array($this->type,array('discount','import'))){ ?>
										<a href="<?php echo hikashop_completeLink('product&task=addrelated&cid='.$row->product_id.'&select_type='.$this->type,true); ?>">
									<?php }?>
