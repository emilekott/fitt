...
class plgHikashoppaymentAtos extends JPlugin
{
  var $accepted_currencies = array(
		...
		if(!empty($methods)){
			foreach($methods as $method){
				...
				if(@$method->payment_params->period<100 && @$method->payment_params->period>0 && @$method->payment_params->instalments<=3 && @$method->payment_params->instalments>=2 && @$method->payment_params->force_instalments==0){
					$method->custom_html='<span style="margin-left:10%">'.JHTML::_('select.booleanlist', "hikashop_multiple_instalments", '',  '', JText::sprintf( 'PAYMENT_IN_X_TIME' , $method->payment_params->instalments ), JText::sprintf( 'PAY_FULL_ORDER' , '1') ).'</span>';
				}
    ...
    function onAfterOrderConfirm(&$order,&$methods,$method_id){
		...
		"amount" => str_replace(array('.',','),'',round($order->cart->full_total->prices[0]->price_value_with_tax*100,(int)$currency->currency_locale['int_frac_digits'])),
		);
    ...
    function onPaymentNotification(&$statuses){
		...
		if($safe_mode){
			...
			$response_path=JPATH_ROOT.DS.$element->payment_params->upload_folder_relative.'b'.DS;
		}
		...
		if($vars['response_code']=='17'){
			$order->order_status = $element->payment_params->invalid_status;
			$order->history->history_data = ob_get_clean().JText::sprintf( 'ORDER_CANCEL_BY_USER');
		...
		$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION','Atos',$vars['payment_status']));
    ...
    function onPaymentConfiguration(&$element){
    	if(JRequest::getCmd('subtask','')=='logos'){
     	...
     	else
      	{
		    ...
		    $bar->appendButton( 'Pophelp','payment-atos-form');
    ...
    function _generateFiles(&$element){
		...
		$safe_mode = ini_get('safe_mode') == 1 || !strcasecmp(ini_get('safe_mode'), 'On');
		if(str_replace(JPATH_ROOT,'',$element->payment_params->upload_folder)!=$element->payment_params->upload_folder_relative)
		...
		$atos='<?php
			...
			include(\'index.php\');
		';
		$success='<?php header("Location: '.hikashop_completeLink('checkout&task=after_end').'");';
