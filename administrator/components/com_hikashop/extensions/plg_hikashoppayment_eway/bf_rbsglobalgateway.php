 ...
 * @subpackage Payment Plug-in for RBS Worldpay Global Gateway using XML Redirect.
 * @version		 0.0.1
 ...
 *
 * See: http://www.rbsworldpay.com/support/kb/gg/submittingtransactionsredirect/rxml.html
 *
 * In order to configure and use this plug-in you must have a RBS Worldpay Global Gateway account.
 * RBS Worldpay Global Gateway is sometimes referred to as 'BiBit'.
 */
...
class plgHikashoppaymentbf_rbsglobalgateway extends JPlugin {
    ...
    function onBeforeOrderCreate(&$order,&$do) {
		...
		if($order->order_payment_method!='bf_rbsglobalgateway') return;
		...
		if(!function_exists('curl_init')){
    		$app =& JFactory::getApplication();
			$app->enqueueMessage('The RBS WorldPay Global Gateway payment plugin needs the CURL library installed but it seems that it is not available on your server. Please contact your web hosting to set it up.','error');
			$do = false;
		}
		if(!class_exists('SimpleXMLElement')){
			$app =& JFactory::getApplication();
			$app->enqueueMessage('The RBS WorldPay Global Gateway payment plugin needs the SimpleXML library of PHP5 installed but it seems that it is not available on your server. Please contact your web hosting to set it up.','error');
			$do = false;
		}
    }
    ...
    function onPaymentNotification(&$statuses){
      ...
      if (@$vars['paymentStatus'] != 'CANCELLED') {
    	 	...
    	 	if($price_check != $paymentAmount || $currency->currency_code != @$vars['paymentCurrency']){
    	 		...
    	 		$mailer->setSubject(JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','RBS Worldpay Global Gateway').JText::_('INVALID_AMOUNT'));
    			$body = str_replace('<br/>',"\r\n",JText::sprintf('AMOUNT_RECEIVED_DIFFERENT_FROM_ORDER','RBS Worldpay Global Gateway',$order->history->history_amount,$price_check.$currency->currency_code))."\r\n\r\n".$order_text;
    			$mailer->setBody($body);
    			$mailer->Send();
    	 		return false;
    	 	}
      ...
      if (!empty($order_status)) $order->order_status = $order_status;
  	 	...
  	 	$mailer->setSubject(JText::sprintf('PAYMENT_NOTIFICATION','RBS Worldpay Global Gateway',$payment_status));
  		$body = str_replace('<br/>',"\r\n",JText::sprintf('PAYMENT_NOTIFICATION_STATUS','RBS Worldpay Global Gateway',$vars['paymentStatus'])).' '.JText::sprintf('ORDER_STATUS_CHANGED',$order->mail_status)."\r\n\r\n".$order_text;
  		$mailer->setBody($body);
  		$mailer->Send();
  	 	$orderClass->save($order);
      $dbg = null;
    ...
    function onPaymentConfiguration(&$element){
  		$this->bf_rbsglobalgateway = JRequest::getCmd('name','bf_rbsglobalgateway');
			if(empty($element)){
				$element = null;
  			$element->payment_name='RBS Worldpay Global Gateway';
        ...
        $element->payment_params->billingNotice="Your payment will be handled by WorldPay.\nThis name may appear on your bank statement\nhttp://www.worldpay.com";
  			$element->payment_params->xmlurl='https://MERCHANTCODE:password@secure-test.wp3.rbsworldpay.com/jsp/merchant/xml/paymentService.jsp';
    	...
    	$bar = & JToolBar::getInstance('toolbar');
			...
			$bar->appendButton( 'Pophelp','payment-bf_rbsglobalgateway-form');
			hikashop::setTitle('RBS Worldpay Global Gateway','plugin','plugins&plugin_type=payment&task=edit&name='.$this->bf_rbsglobalgateway);
			$app =& JFactory::getApplication();
