...
class hikashopOrderHelper{
	...
	function save(){
		...
		while(!empty($order) OR !empty($results)){
			...
			if(!empty($order) && empty($dbElement->$orderingMap) OR (!empty($order) AND reset($order) <= $dbElement->$orderingMap)){
		...
		foreach($newOrder as $val){
			...
			if(!isset($oldResults[$val]) OR $oldResults[$val]->$orderingMap != $i){
