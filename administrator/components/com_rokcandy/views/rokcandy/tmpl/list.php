...
window.addEvent('domready', function() {
	var parent = document.id(window.parent.document.body);
	var macros = document.id('editcell').getElements('tbody tr').getFirst(), text = parent.getElement('#<?php echo JRequest::getVar('textarea', 'text'); ?>'), sbox = parent.getElement('#sbox-window');
	if (macros.length) {
...
<div id="editcell">
	...
	for ($i=0, $n=count( $this->items ); $i < $n; $i++)
	{
		...
		$row->cat_link 	= JRoute::_( 'index.php?option=com_categories&extension=com_rokcandy&task=edit&type=other&cid[]='. $row->catid );
