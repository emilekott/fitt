...
class GantryLayoutMod_Scroller extends GantryLayout {
    ...
    function render($params = array()){
        ...
        // XHTML LAYOUT
?>
<?php if (controlsDisplay($value) && !$this->_getScrollerCache($value)): ?>
<div class="controls">
...
<?php
		...
		$this->_setScrollerCache($value);
		
        return ob_get_clean();
		
    }

	function _getScrollerCache($value = false){
        global $gantry;
		$cache = $gantry->retrieveTemp('template', 'scroller');
		if (!$cache) $cache = array();
		
		$gantry->addTemp('template', 'scroller', $cache);
		return (!$value) ? $gantry->retrieveTemp('template', 'scroller') : in_array($value, $cache);
	}
	
	function _setScrollerCache($value){
        global $gantry;
		$cache = $gantry->retrieveTemp('template', 'scroller');
		$cache[] = $value;
